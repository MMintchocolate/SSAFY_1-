...
function goStart() {
  router.push(localStorage.getItem("access") ? "/app/home" : "/home");
}
...
<template>
  <div class="page">
    ...
    <section class="intro">
      ...
      <div
        class="cloud cloud-yellow hero-cloud-1"
        :class="{ show: cloudOn }"
      ></div>
      <div
        class="cloud cloud-mint hero-cloud-2"
        :class="{ show: cloudOn }"
      ></div>

      <div class="center-block">
        <p class="descriptor" :class="{ show: cloudOn }">
        ...
        <h1 class="brand">
          <span
            v-for="(l, i) in ['m', 'o', 'n', 'i']"
            :key="i"
            class="bltr"
            :class="{ show: lettersOn > i }"
            >{{ l }}</span
          >
        </h1>
      ...
      >
        ...
        <div class="feat-text">
          <span
            ...
            >{{ item.badge }}</span
          >
          <p class="feat-label" :style="`color:${item.badgeColor}`">
            {{ item.label }}
          </p>
          ...
          <ul class="feat-chips">
            <li
              ...
              :key="f"
              class="feat-chip"
              :style="`border-color:${item.accent}55;color:${item.badgeColor}`"
            >
              <span class="feat-dot" :style="`background:${item.accent}`"></span
              >{{ f }}
            </li>
          </ul>
          <RouterLink
            :to="item.to"
            class="feat-btn"
            :style="`background:${item.accent};color:#111827`"
          >
            바로가기 →
          </RouterLink>
        </div>
        ...
        <div class="feat-shot">
          <div class="shot-wrap" :style="`--accent:${item.accent}`">
            ...
            <div class="shot-placeholder">
              ...
              <p class="ph-hint">
                fronted/public/screenshots/{{ item.id }}.png
              </p>
            </div>
          </div>
        </div>
    ...
    <section class="simple-wrap">
      <div class="features-header">
        ...
        <h2 class="features-title">
          모든 금융 정보를 <em class="accent-yellow">한 곳에서</em>
        </h2>
      </div>
...
.bltr {
  ...
  transition: opacity 0.45s ease,
    transform 0.45s cubic-bezier(0.34, 1.3, 0.64, 1), filter 0.45s ease;
}
.bltr.show {
  opacity: 1;
...
.features-title em.accent-yellow {
  color: #ffc62a;
}
.features-sub {
  font-size: 0.95rem;
...
.shot-wrap.no-image .shot-placeholder {
  display: flex;
}
.shot-wrap:not(.no-image) .shot-placeholder {
  display: flex;
}
.shot-wrap:not(.no-image) .shot-img {
  position: absolute;
  inset: 0;
}
.ph-icon {
